 ...
 *
 * File version:     1.8
 * File mod date:    2005-01-20 20:22:07
...
UINT_32 crc_hash_int2( UINT_32 x, UINT_32 y )
{
  ...
  ch = x & 0xFF;             HASH_IN(ch);
  ch = (x >> 8) & 0xFF;      HASH_IN(ch);
  ch = (x >> 16) & 0xFF;     HASH_IN(ch);
  ch = (x >> 24) & 0xFF;     HASH_IN(ch);

  ch = y & 0xFF;             HASH_IN(ch);
  ch = (y >> 8) & 0xFF;      HASH_IN(ch);
  ch = (y >> 16) & 0xFF;     HASH_IN(ch);
  ch = (y >> 24) & 0xFF;     HASH_IN(ch);
  return h;
}
