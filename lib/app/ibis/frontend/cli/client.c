...
#define DEFAULT_SERVICE_PORT            5150
char *program = "a.out";
...
/*
 *  Check whether we want to allow the server to access
 *  the given file.
 *
 *  XXX  Should do something slightly secure here.  
 *
 *     Idea:
 *       As a function of the invoked command, we can
 *       indicate which parameters are supplying filenames
 *       to the server.
 *
 *     Idea:
 *       The server never opens files by name; it references
 *       arguments 
 *
 *  Problems...  Directory-based output + what if someone provides
 *               an argument called "/"?  The server can pretend
 *               that's a directory name.
 *
 */

int authenticate_file_access( const char *file, int mode )
{
  return 1;             /* for now, we trust the server */
}
...
{
  static const char *(temps[20]);               /* EVIL! */
  static int num_temps = 0;
  struct argv_list *c;

  assert( num_temps < 20 );
  temps[num_temps] = value;
  c = alloc_argv_list( key, &temps[num_temps] );
  c->num_values = 1;
  num_temps++;
}

const char *find_arg_in_cfg( const char *name )
{
  FILE *f;
  char file[1024];
  const char *h;
  ...
  if (f) {
    ...
    while (fgets( buf, sizeof(buf), f )) {
      ...
      if ((strncmp( buf, name, strlen(name) ) == 0)
          && (buf[strlen(name)] == '=')) {
        fclose( f );
        return strdup( &buf[strlen(name)+1] );
      }
    }
    fclose( f );
  }
...
/*
 *  Returns:
 *     2 if the value was supplied on the command line (`--key value')
 *     1 if the argument was found in the environment ("IBIS_KEY=value")
 *     0 otherwise
 */

int import_arg_from_env( const char *name, const char *env_name, int req )
{
  if (!defined(name))
    {
      const char *s;
...
int main( int argc, const char **argv )
{
  ...
  logname = pw->pw_name;
   ...
   optional_arg( "login", "IBIS_LOGIN" );
   required_arg( "server", "IBIS_SERVER" );
...
{
  char temp[1024];
  sprintf( temp, "%s/%s", client_top, (char*)(argv[0].str.addr) );
...
void upload_cmd( union cmd_arg *argv )
{
  ...
  if (!authenticate_file_access( path, 'r' )) {
    fprintf( stderr, "%s: Server access denied\n", path );
    exit(1);
  }
