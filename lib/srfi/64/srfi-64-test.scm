    ...
    (test-runner-on-test! 
     ...
     (lambda (runner result)
       (let ((n (test-runner-test-name runner)))
         (case (cdr (assq 'test-kind result))
...
(test-equal
 "2.1.3. PASS with a test name"
 (triv-runner
  (lambda ()
    ;; PASS
    (test-error "a" (vector-ref '#(1 2) 9))))
 '(("a") () () () () (1 0 0 0 0)))

(test-equal
 "2.1.4. PASS with a test name and error type"
 (triv-runner
  (lambda ()
    ;; PASS
...
(test-equal
 "3.4. test-begin with name and count on test-end"
 (triv-runner
  (lambda ()
    (test-begin "a")
    (test-assert "b" #t)
...
(test-error
 "3.5. test-begin with mismatched test-end count"
 (triv-runner
  (lambda ()
    (test-begin "a")
    (test-assert "b" #t)
...
(test-assert "8.2.1. default on-test hook"
             (eq? (test-runner-on-test (test-runner-simple))
                  test-on-test-simple))
...
(define (auxtrack-runner thunk)
  (let ((r (test-runner-null)))
    ...
    (test-runner-on-test! r (lambda (r x)
...
(test-equal (prop-runner
             '(kind expected-result actual-result)
             (lambda ()
               (test-equal (+ 1 2) 2)))
            '((kind . fail)
              (expected-result . 2)
...
(test-equal (on-test-runner
             (lambda ()
               (test-assert #t))
             (lambda (r)
               (test-result-ref r 'kind)))
            '(pass))
