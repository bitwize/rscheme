#|------------------------------------------------------------*-Scheme-*--|
 | File:	    rs/backend/c/parseglue.scm
 |
...
(define (parse-glue-body form lex-envt dyn-envt)
  ...
  (let ((template-flag? #f)
	  ...
	  ((type-handler:)
           (let ((h (cadr f)))
             (if (and (>= (length h) 3)
                      (pair? (caddr h))
                      (eq? (caaddr h) 'lambda))
                 (set! h (compile-glue-type-handler h)))
             (set! type-handlers (cons h type-handlers))))
	...
	(loop (cddr f)))
       ...
       ;; they consist of the flag by itself
       ((flag? (car f))
	(case (car f)
	  ((:template)
	   (set! template-flag? #t))
...
;;;  where
;;;
;;;     CHECKER-SPEC ::= RECOGNITION-STYLE VIEW-SPEC ...
;;;
...
;;;  view-decl can be (X Y Z ...)
;;;  in which case, the view name is (format #f Z basename)
...
;;; apply the VIEW-DECL to the passed argument, to arrange
;;; for the extraction of the necessary pieces
...
(define (compile-glue-type-handler h)
  (list (car h)
        (cadr h)
        (eval (caddr h))))
